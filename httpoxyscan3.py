...
import urllib.request, urllib.parse, urllib.error, urllib.request, urllib.error, urllib.parse, sys, getopt, requests, \
    ssl, time, sys, subprocess, os
from array import *
...
def main(argv):
    ...
    if argc < 5:
        ...
        print(bcolors.WARNING + "[*] Usage: %s http://target.com cgi_list.txt listener_ip listener_port" % (
        argv[0]) + bcolors.ENDC)
        print("")
        sys.exit(0)
